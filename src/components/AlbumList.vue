  ...
  <div>
    <div class="top">
      ...
      <div class="top_content" v-if="lists">
        <div class="img">
          <img :src="lists.album.picUrl" alt="" />
        </div>
        <div class="top_right">
          <span class="tops">{{ lists.album.name }}</span>
          <div class="right">
            <div class="img_one">
              <img :src="lists.album.artist.img1v1Url" alt="" />
            </div>
            <span>{{ lists.album.artist.name }}</span>
          </div>
        </div>
      </div>
      <div class="bottom">
        <span class="iconfont icon-xinaixin"
          ><p>{{ songListarr.subCount | news }}</p></span
        >
        <span class="iconfont icon-pinglun" @click="stopComs = true"
          ><p>{{ songListarr.commentCount }}</p></span
        >
        <span class="iconfont icon-xiazai"><p>全部下载</p></span>
        <span class="iconfont icon-fenxiang"
          ><p>分享({{ songListarr.shareCount | news }})</p></span
        >
      </div>
      <div class="all" @click="pushs">
        <span class="all_span iconfont icon-play-circle"
          ><span>播放全部 ({{ allList.length }})</span></span
        >
      </div>
    </div>
    <loading v-if="allList.length < 1"></loading>
    <li-item
    ...
    >
      <div class="comment" v-show="stopComs">
        ...
        <div class="hotlists">
          ...
          <span class="actived" v-if="hotCommentsarr.length == 0"
            >暂无热门评论</span
          >
          <ul>
...
export default {
  ...
  methods: {
    ...
    plus() {
      ...
      this.commentsarr = this.commentsarr.concat(
        this.commentsarrs.slice(start, end)
      );
    },
    pushs() {
      this.$emit("res", this.allList);
    },
  },
  beforeRouteEnter(to, from, next) {
    next((vm) => {
      vm.$nextTick(() => {
        vm.$axios.get("/album?id=" + vm.id).then((d) => {
          vm.lists = d.data;
          vm.allList = d.data.songs.map((data) => {
            data.song = {
              artists: data.ar,
              album: data.al,
            };
        ...
        vm.$axios.get("/album/detail/dynamic?id=" + vm.id).then((d) => {
            console.log(d.data);
          vm.songListarr = d.data;
        });
        vm.$axios.get("/comment/album?id=" + vm.id + "&limit=100").then((d) => {
